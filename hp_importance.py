...
def analyze_hyperparameter_sensitivity(
    ...
    for hp_value in hp_values:
            ...
            if hp_name == "d_model":
                ...
                if hp_value % nhead != 0:
                    ...
                    if compatible_nhead:
                        ...
                        print(
                            f"  Adjusted nhead to {compatible_nhead} for compatibility")
                    else:
                        print(
                            f"  Skipping: d_model={hp_value} not divisible by any valid nhead")
                        skip_value = True
            elif hp_name == "nhead":
                ...
                if d_model % hp_value != 0:
                    ...
                    print(
                        f"  Skipping: d_model={d_model} not divisible by nhead={hp_value}")
                    skip_value = True
        ...
        # Longer training for Transformer
        max_epochs = 30 if model_type == "transformer" else 10
        patience = 5 if model_type == "transformer" else 2
...
def run_sensitivity_analysis(
    ...
    # CRITICAL: Verify Transformer is fixed
    if model_type == "transformer":
        best_f1 = max(max(result["f1_scores"]) for result in results.values())
        assert best_f1 > 0.75, f"Transformer still broken! Best F1={best_f1:.2f} < 0.75"
        print(f"✓ TRANSFORMER FIXED: Best validation F1 = {best_f1:.2f}")
...
if __name__ == '__main__':
    ...
    # Use larger subset for better Transformer training
    # Increased from 1000 to 5000 for better learning
    ...
    train_loader_trans = DataLoader(
        train_dataset_trans, batch_size=32, shuffle=True)
    val_loader_trans = DataLoader(
        val_dataset_trans, batch_size=32, shuffle=False)

    train_loader_lstm = DataLoader(
        train_dataset_lstm, batch_size=32, shuffle=True)
    val_loader_lstm = DataLoader(
        val_dataset_lstm, batch_size=32, shuffle=False)
